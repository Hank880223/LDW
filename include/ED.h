...
enum GradientOperator { PREWITT_OPERATOR = 101, SOBEL_OPERATOR = 102, SCHARR_OPERATOR = 103, LSD_OPERATOR = 104 , OUR_OPERATOR = 105};
...
public:
	...
	cv::Mat getGradImage();
	cv::Mat gettestImage();
	cv::Mat getSelectGradImage();
	
	cv::Mat H(int ps1x, int ps1y, int ps2x, int ps2y, int ps3x, int ps3y, int ps4x, int ps4y,
			  int pd1x, int pd1y, int pd2x, int pd2y, int pd3x, int pd3y, int pd4x, int pd4y);
	double line_width(double x_data, double y_data, double z_data);
	...
	cv::Mat m = cv::Mat::ones(3,1,CV_64FC1);
...
protected:
	...
	uchar *anchorsImg; // pointer to edge image data
	uchar *smoothImg; // pointer to smoothed image data
	uchar *testImg; // pointer to smoothed image data
	int segmentNos;
	int minPathLen;
	cv::Mat srcImage;
...
private:
	...
	void ModifiedGaussian(const cv::Mat &src, cv::Mat &dst, int ksize, double sigma);
	void ComputeAnchorPoints();
	void OComputeAnchorPoints();
	void ComputeLaneLineWidthAnchorPoints();
	void JoinAnchorPointsUsingSortedAnchors();
	void ComputeLaneLineWidth();
	...
	cv::Mat anchorsImage;
	cv::Mat gradImage;
	cv::Mat selectgradImage;
	cv::Mat gradlinewidthImage;
	cv::Mat testImage;

	uchar *dirImg; // pointer to direction image data
	
	short *gradImg; // pointer to gradient image data
	short *selectgradImg; // pointer to gradient image data
	short *gradlinewidthImg;
	...
	//cv::Mat HomographyMatrix = cv::Mat::ones(3, 3, CV_64FC1);
